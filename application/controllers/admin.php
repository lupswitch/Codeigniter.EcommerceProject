	...
	{			
		...
		else 
		{
				...
				else 
				{
					...
					include("/application/libraries/php_image_magician.php");
					
					
					$type		=	substr(strtolower($_FILES['file']['name']),-4);
					$img_name	= 	substr($this->input->post('title'),0,40);
					$img_name 	= 	str_replace(" ","_",$img_name)."_".$id;
					
					$big_image 	=	"img/productos/".$img_name."_big".$type;
					$med_image 	=	"img/productos/".$img_name."_med".$type;
					$thumbnail 	=	"img/productos/".$img_name."_small".$type;
								
						copy($_FILES['file']['tmp_name'],$big_image);
						
							$magicianObj = new imageLib($big_image);
							$magicianObj -> resizeImage(75, 75);
							$magicianObj -> saveImage($thumbnail, 99);
							
							$magicianObj = new imageLib($big_image);
							$magicianObj -> resizeImage(200,200,'auto');
							$magicianObj -> saveImage($med_image, 99);
							
							$magicianObj = new imageLib($big_image);
							$magicianObj -> resizeImage(400,400,'auto');
							$magicianObj -> saveImage($big_image, 99);
	...
	{			
		...
		else 
		{
				...
				else 
				{
					...
					$img_name 	= 	str_replace(" ","_",$img_name) ."_". $id;
					$big_image 	=	"img/productos/".$img_name.".".$type;
					$med_image 	=	"img/productos/".$img_name."_med.".$type;
					$thumbnail 	=	"img/productos/".$img_name."_small.".$type;
					...
					copy($_FILES['file']['tmp_name'],$big_image);
						...
						$magicianObj = new imageLib($big_image);
						$magicianObj -> resizeImage(75, 75);
						$magicianObj -> saveImage($thumbnail, 100);
						
							$magicianObj = new imageLib($big_image);
							$magicianObj -> resizeImage(220, 220);
							$magicianObj -> saveImage($med_image, 100);
							
							$magicianObj = new imageLib($big_image);
							$magicianObj -> resizeImage(300, 300);
							$magicianObj -> saveImage($big_image, 100);
