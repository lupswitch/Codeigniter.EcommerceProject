...
class cart_model extends CI_Model {
    ...
    function __construct()
    {
		...
		function remove_all($sess){
			//delete from cart where cart_session = $cart_session
			$this->db->where('cart_session',$sess);
			$this->db->delete('cart');
		}
		...
		function update_quantity_in_cart($item_id,$qty){
			$query = "update cart set item_total_price = (item_price*$qty),item_quantity='$qty' where item_id = '$item_id' and cart_session='".$this->session->userdata('cart_session')."'";
			$this->db->query($query);
		}
